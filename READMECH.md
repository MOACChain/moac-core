...
本次发布的更新仅限于VNODE客户端，包含对MOAC主网 mainnet 的 VNODE 节点的智能合约运行环境VM进行重大升级，新增对以下opcode的支持。升级后，VNODE 节点可最高支持运行solidity 0.8编译器编译的合约字节码。按照发布计划，mainnet升级后分叉高度为6435000区块，预计分叉时间为北京时间3月5日前后（周五）。
新增的opcode如下：
* SHL
* SHR
* SAR
